...
python3 train_bbh_symbolic.py "${COMMON_ARGS[@]}" \
  ...
  --log-jsonl runs_bbh/logs/20_walk_trace_transformer_"$MODEL_SIZE"_"$RUN_ID".jsonl

python3 train_bbh_symbolic.py "${COMMON_ARGS[@]}" \
  --architecture memory_update \
  --n-pass "$N_PASS" \
  --memory-update-gate on \
  --log-jsonl runs_bbh/logs/20_walk_trace_memory_update_"$MODEL_SIZE"_"$RUN_ID".jsonl
