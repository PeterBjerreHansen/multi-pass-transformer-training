...
def test_generation_aligned_loss_reuses_prefix_and_trains_recurrent_path():
    vocab, stoi, _ = shortest_path.build_shortest_path_vocab("easy")
    block_size = shortest_path.required_block_size("easy")
...
def test_generation_aligned_loss_sampling_is_deterministic():
    num_nodes = 7
    num_hops = 3
    vocab, stoi, _ = pointer_chasing.build_pointer_chasing_vocab(num_nodes)
    block_size = pointer_chasing.required_block_size(num_nodes, num_hops)
    model = MemoryTapeTransformer(
        MemoryTapeConfig(block_size, len(vocab), 1, 1, 8, 3)
    )
    batch = pointer_chasing.build_pointer_chasing_batch(
...
def test_generation_aligned_rng_checkpoint_resume_reproduces_next_step(tmp_path):
    ...
    config = MemoryTapeConfig(
        pointer_chasing.required_block_size(num_nodes, num_hops),
        len(vocab),
        1,
        1,
        8,
